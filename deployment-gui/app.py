...
def construct_cmd(
    ...
    match container_type:
        case "docker":
            cmd += ["docker", "run",
                        ...
                        "--gpus", "all",
                        "--shm-size", "1g",
                        "-v", f"{model_dir.absolute()}:/data",
                        "ghcr.io/huggingface/text-generation-inference:latest"]
        case "singularity":
            cmd += ["singularity", "run",
                        "--nv",
                        "--mount", f"type=bind,src={model_dir.absolute()},dst=/data",
                        "--env", "HF_HOME=/data",
    ...
    cmd += [f"--max-total-tokens={context_length}",
...
def main():
    ...
    with gr.Blocks() as app:
        ...
        modelDropdown.select(
            lambda choice: (choice, " ".join(construct_cmd(
                model_name=modelDropdown.value,
                model_dir=model_directory,
                quantization=qtzDropdown.value))),
        ...
        previewBtn.click(
            fn=lambda x: " ".join(construct_cmd(
                model_name=modelDropdown.value,
                model_dir=model_directory,
                quantization=qtzDropdown.value,
            )),
